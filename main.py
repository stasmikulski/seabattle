...
class Field(object):
    ...
    def recalculate_weight_map(self, available_ships):
        ...
        # Это открытая инфа исходя из правил игры.  Проходим по каждой клетке поля.
        # Если там уничтоженый корабль или клетка с промахом -
...
class Game(object):
    #Вот строка для поля 6х6:
    letters = ("A", "B", "C", "D", "E", "F")
    #Чтобы не переписывать вот строка для поля 10х10:
    #letters = ("A", "B", "C", "D", "E", "F", "G", "H", "I", "J")
    #Вот корабли для поля 10х10:
    #ships_rules = [4, 3, 3, 2, 2, 2, 1, 1, 1, 1]
    #ships_rules = [1, 1, 1, 1, 2, 2, 2, 3, 3, 4]
    #Вот корабли для поля 6х6:
        ...
        for ship_size in Game.ships_rules:
            ...
            while True:
                ...
                if player.auto_ship_setup is not True:
                    ...
                    player.message.append('Куда поставить {}-трубный корабль:'.format(ship_size))
                    for _ in player.message:
                        print(_)
                else:
                    #print('{}. Расставляем корабли...'.format(player.name))
                    count_real_retry += 1
                    #print(count_real_retry)
                ...
                # или просто просим еще раз ввести координаты, напоминаем формат ввода
                if x + y + r == 0:
                    player.message.append('Формат ввода: a1v или a1h, где v - вертикально, h - горизонтально')
                    continue
    ...
    def draw(self):
        if not self.current_player.is_ai:
            print('Ваше поле, {}'.format(game.current_player.name))
            self.current_player.field.draw_field(FieldPart.main)
            print('Ваш радар (поле игрока {})'.format(game.next_player.name))
...
if __name__ == '__main__':
    ...
    players = []
    players.append(Player(name='Stas', is_ai=False, auto_ship=False, skill=1))
    players.append(Player(name='Computer', is_ai=True, auto_ship=True, skill=1))
